...
def create_bingx_order(api_key, secret_key, symbol, side, order_type, quantity, tp_price=None, sl_price=None):
    """Membuat order di BingX dengan metode pembuatan signature yang paling robust."""
    ...
    params = {
        ...
        # Penting: ubah angka menjadi string dengan presisi yang wajar untuk konsistensi
        'quantity': f"{quantity:.8f}", 
        'timestamp': int(time.time() * 1000)
    }
    
    # Tambahkan TP/SL jika ada, format juga sebagai string
    if tp_price: params['takeProfit'] = f"{tp_price:.5f}"
    if sl_price: params['stopLoss'] = f"{sl_price:.5f}"

    # Gunakan urllib.parse.urlencode untuk membuat string query yang 100% benar
    # Ini adalah kunci perbaikannya
    query_string = urllib.parse.urlencode(sorted(params.items()))
    
    # Buat signature dari query string yang sudah benar
    signature = generate_bingx_signature(secret_key, query_string)
    
    # Kirim request. Gabungkan query string dan signature di URL
    headers = {'X-BX-APIKEY': api_key}
    full_url = f"{BINGX_API_URL}{endpoint}?{query_string}&signature={signature}"
    ...
    try:
        ...
        if data.get('code') == 0:
            ...
            # Berikan pesan error yang detail dari BingX
